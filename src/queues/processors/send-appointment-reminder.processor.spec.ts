...
function jobFor(targets: unknown[]) {
  return { name: SEND_APPOINTMENT_REMINDER_JOB, data: { targets } } as Job<SendAppointmentReminderJob>;
}
...
describe('SendAppointmentReminderProcessor', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [SendAppointmentReminderProcessor, { provide: MailService, useValue: mailService }],
    }).compile();
  ...
  it('does nothing for a non-matching job name', async () => {
    const job = { name: 'some_other_job', data: { targets: [target()] } } as unknown as Job<SendAppointmentReminderJob>;
